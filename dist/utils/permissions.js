...
export async function hasAdminAccess(member) {
    const settings = await settingsRepository.get(member.guild.id);
    if (member.id === member.guild.ownerId || member.id === settings.bot.ownerId || member.permissions.has(PermissionFlagsBits.Administrator))
        return true;
...
export async function hasStaffAccess(member) {
