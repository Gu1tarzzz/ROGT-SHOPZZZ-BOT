import { ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, ChannelSelectMenuBuilder, ChannelType, EmbedBuilder, type GuildTextBasedChannel } from "discord.js";
  ...
  return new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        ...
        { label: "ดีไซน์หลังร้าน", value: "backoffice", description: "รูปลักษณ์แดชบอร์ด", emoji: UI_EMOJI.component.image },
        ...
        { label: "ยอดผู้ใช้", value: "user-balance", description: "จัดการยอดเงินและคะแนน", emoji: UI_EMOJI.component.payment },
...
export function sectionButtons(section: "appearance" | "backoffice" | "payment" | "tickets" | "bot" | "user-balance"): ActionRowBuilder<ButtonBuilder> {
  const map = {
    appearance: [
 ...
 */
export function backOfficeDesignEmbed(guildId: string, settings: Awaited<ReturnType<typeof settingsRepository.get>>): { embed: EmbedBuilder, components: ActionRowBuilder<any>[] } {
  ...
  return { 
    ...
    components: [
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        ...
        new ButtonBuilder().setCustomId("setup:preview:dashboard").setLabel("ดูตัวอย่าง").setStyle(ButtonStyle.Secondary).setEmoji(UI_EMOJI.component.browse)
      )
    ] 
  };
}
 ...
 */
export function userBalanceManagerEmbed(guildId: string): { embed: EmbedBuilder, components: ActionRowBuilder<any>[] } {
  const embed = new EmbedBuilder()
    .setColor("#8B5CF6")
  ...
  return {
    ...
    components: [
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("setup:modal:user-balance:add-balance").setLabel("Add Balance").setStyle(ButtonStyle.Success).setEmoji(UI_EMOJI.component.payment),
        new ButtonBuilder().setCustomId("setup:modal:user-balance:remove-balance").setLabel("Remove Balance").setStyle(ButtonStyle.Danger).setEmoji(UI_EMOJI.component.remove),
        new ButtonBuilder().setCustomId("setup:modal:user-balance:check-balance").setLabel("Check Balance").setStyle(ButtonStyle.Secondary).setEmoji(UI_EMOJI.component.analytics)
      )
    ]
  };
}
 ...
 */
export function bankSetupEmbed(guildId: string, settings: Awaited<ReturnType<typeof settingsRepository.get>>): { embed: EmbedBuilder, components: ActionRowBuilder<any>[] } {
  const payment = settings.payment;
  const bankName = payment.bank?.bankName || "—";
  const accountName = payment.bank?.accountName || "—";
  const accountNumber = payment.bank?.accountNumber || "—";
  ...
  return { 
    ...
    components: [
      new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("payment:bank:edit").setLabel("Edit Bank").setStyle(ButtonStyle.Primary).setEmoji(UI_EMOJI.component.edit),
        new ButtonBuilder().setCustomId("payment:qr:edit").setLabel("Edit QR").setStyle(ButtonStyle.Primary).setEmoji(UI_EMOJI.component.image)
      )
    ] 
  };
}
 ...
 */
export function notificationSetupEmbed(guildId: string, settings: Awaited<ReturnType<typeof settingsRepository.get>>, guildChannels?: GuildTextBasedChannel[]): { embed: EmbedBuilder, components: ActionRowBuilder<any>[] } {
  const payment = settings.payment;
  const topupChannel = payment.logs?.topupChannel ? `<#${payment.logs.topupChannel}>` : "—";
  const purchaseChannel = payment.logs?.purchaseChannel ? `<#${payment.logs.purchaseChannel}>` : "—";
